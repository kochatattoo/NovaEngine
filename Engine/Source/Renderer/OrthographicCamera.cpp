...
namespace NK {
	...
	glm::vec2 OrthographicCamera::ScreenToWorldPoint(int screenX, int screenY, uint32_t windowWidth, uint32_t windowHeight) const {
		...
		// ƒл€ игровой камеры left/right/bottom/top
	...
	glm::vec2 OrthographicCamera::ScreenToUIPoint(int screenX, int screenY, uint32_t windowWidth, uint32_t windowHeight) const {
		float ndcX = (float)screenX / windowWidth;
		float ndcY = 1.0f - (float)screenY / windowHeight;   // Y вниз дл€ UI
		float worldX = m_Left + (m_Right - m_Left) * ndcX;
		float worldY = m_Bottom + (m_Top - m_Bottom) * ndcY;
		return glm::vec2(worldX, worldY);
	}
	...
	void OrthographicCamera::OnWindowResized(uint32_t width, uint32_t height) {
		// —охран€ем соотношение сторон так, чтобы видима€ область сохран€ла нужный размер по высоте, а ширина подстраивалась
		float aspectRatio = (float)width / (float)height;
		// ѕредположим, мы хотим видеть 10 единиц по вертикали
