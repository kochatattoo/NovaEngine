...
namespace NK::Game::ECS {
    ...
    Match3System::Match3System(NK::ECS::World& world,
                               int rows, int cols,
                               float cellSize, float pixelsPerUnit)
        : m_World(world)
        , m_Rows(rows)
        , m_Cols(cols)
        , m_CellSize(cellSize)
    ...
    void Match3System::Start() {
        ...
        // (Порядок: сначала создаём все entity, потом заполняем grid и обновляем.)
        ...
        FillRandom();
        NK_CORE_INFO("Match3System: spawned %d tile entities in World", m_Rows * m_Cols);
    }
    ...
    void Match3System::UpdateTileEntity(int row, int col, TileType newType) {
        entt::entity e = GetTileEntity(row, col);
    ...
    std::vector<std::pair<int, int>> Match3System::FindMatches() const {
        ...
        for (int r = 0; r < m_Rows; ++r) {
            for (int c = 0; c < m_Cols; ++c) {
                if (marked[r][c]) matches.emplace_back(r, c);
            }
        }
    ...
    void Match3System::Mix() {
        ...
        if (!HasPossibleMoves()) {
            NK_CORE_WARN("Match3System::Mix: failed to find a state with possible moves after %d attempts", maxAttempts);
        }
    }
